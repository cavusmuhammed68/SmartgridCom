# -*- coding: utf-8 -*-
"""
Created on Fri Mar 28 21:33:15 2025
...
def run_pipeline(target, data_dir, save_prefix):
    ...
    plt.step(range(100), y_test_seq[:100], label='Actual', linestyle='--', linewidth=2, color='black', where='mid')
    plt.step(range(100), y_pred_lstm[:100], label='LSTM', where='mid')
    plt.step(range(100), y_pred_rf[:100], label='RF', where='mid')
    plt.step(range(100), y_pred_svm[:100], label='SVM', where='mid')
    plt.step(range(100), y_pred_lr[:100], label='LR', where='mid')
    plt.step(range(100), y_pred_xgb[:100], label='XGB', color='black', where='mid')
    plt.step(range(100), y_pred_hybrid[:100], label='Hybrid', linewidth=2.5, linestyle='-.', color='red', where='mid')
    plt.xlabel("Time Step [hours]", fontsize=16)
    plt.ylabel("Generation [kW]", fontsize=16)
    ...
    plt.tight_layout()
    plt.savefig(os.path.join(data_dir, f"{save_prefix}_forecast_100h_step1.png"), dpi=600)
    plt.show()
    ...
    plt.step(range(100), y_pred_rf[:100], label='RF', where='mid')
    plt.step(range(100), y_pred_svm[:100], label='SVM', where='mid')
    plt.step(range(100), y_pred_xgb[:100], label='XGB', color='magenta', where='mid')
    plt.step(range(100), y_pred_hybrid[:100], label='Hybrid', linewidth=2.5, linestyle='-.', color='red', where='mid')
    plt.xlabel("Time Step [hours]", fontsize=16)
    plt.ylabel("Generation [kW]", fontsize=16)
    plt.title(f"Forecast Comparison: {target} (First 100 Hours)")
    ...
    plt.legend(fontsize=14)
    plt.tight_layout()
    plt.savefig(os.path.join(data_dir, f"{save_prefix}_forecast_100h_step2.png"), dpi=600)
    plt.show()
    ...
    plt.step(range(100), y_pred_rf[:100], label='RF', where='mid')
    plt.step(range(100), y_pred_svm[:100], label='SVM', where='mid')
    plt.step(range(100), y_pred_xgb[:100], label='XGB', color='cyan', where='mid')
    plt.step(range(100), y_pred_hybrid[:100], label='Hybrid', linewidth=2.5, linestyle='-.', color='red', where='mid')
    plt.xlabel("Time Step [hours]", fontsize=16)
    plt.ylabel("Generation [kW]", fontsize=16)
    plt.title(f"Forecast Comparison: {target} (First 100 Hours)")
    ...
    plt.legend(fontsize=14)
    plt.tight_layout()
    plt.savefig(os.path.join(data_dir, f"{save_prefix}_forecast_100h_step3.png"), dpi=600)
    plt.show()
...
def run_pipeline(target, data_dir, save_prefix):
    ...
    metrics['Linear Regression'] = eval_model(y_test_seq, y_pred_lr, "Linear Regression")
    metrics['XGBoost'] = eval_model(y_test_seq, y_pred_xgb, "XGBoost")
    metrics['NeuroFusion'] = eval_model(y_test_seq, y_pred_hybrid, "NeuroFusion")

    # === Forecast Step Plot (No LR, New Label) ===
    plt.figure(figsize=(14, 7))
    plt.step(range(100), y_test_seq[:100], label='Actual', linestyle='--', linewidth=2, color='black', where='mid')
    plt.step(range(100), y_pred_lstm[:100], label='LSTM', where='mid')
    plt.step(range(100), y_pred_rf[:100], label='RF', where='mid')
    plt.step(range(100), y_pred_svm[:100], label='SVM', where='mid')
    plt.step(range(100), y_pred_xgb[:100], label='XGB', color='black', where='mid')
    plt.step(range(100), y_pred_hybrid[:100], label='NeuroFusion', linewidth=2.5, linestyle='-.', color='red', where='mid')
    plt.xlabel("Time Step [hours]", fontsize=16)
    plt.ylabel("Generation [kW]", fontsize=16)
    plt.title(f"Forecast Comparison: {target} (First 100 Hours)")
    plt.xticks(fontsize=16)
    ...
    plt.legend(fontsize=14)
    plt.tight_layout()
    plt.savefig(os.path.join(data_dir, f"{save_prefix}_forecast_100h_step4.png"), dpi=600)
    plt.show()

    # === Scatter Plot for NeuroFusion ===
    plt.figure(figsize=(6, 6))
    plt.scatter(y_test_seq, y_pred_hybrid, alpha=0.5, edgecolors='k')
    plt.plot([0, 1], [0, 1], 'r--')
    plt.xlabel("Actual", fontsize=16)
    plt.ylabel("Predicted", fontsize=16)
    plt.title(f"Scatter: NeuroFusion vs Actual ({target})")
    plt.xticks(fontsize=16)
    plt.yticks(fontsize=16)
    plt.grid(True)
    plt.tight_layout()
